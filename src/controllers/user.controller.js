...
const logoutUser = asyncHandler(async(req,res) => {
    await User.findByIdAndUpdate(
        ...
        {
            $unset:{
                refreshToken: null  // this removes the field from document   use null or undefined
            }
        },
        {
            new: true
...
const updateUserAvatar = asyncHandler(async(req,res) => {
    const avatarLocalPath = req.file?.path   //file not files qki uplaod only one image

    if(!avatarLocalPath){
        throw new ApiError(400, "Avatar file is missing")
    }
 
    // Todo - make a file in utility to delete old image
...
const updateUserCoverImage  = asyncHandler(async(req,res) => {
    const coverImageLocalPath = req.file?.path   //file not files qki uplaod only one image
